...
.SYNOPSIS
  ...
  as a standalone exe (PyInstaller), publishes the agent as a self-contained exe, downloads
  WinSW to wrap the backend as a Windows service, and builds the MSI with WiX.
...
# 4. WinSW (wraps the backend exe as a real Windows service — see ARCHITECTURE.md)
...
New-Item -ItemType Directory -Force -Path "$stagingDir\console", "$stagingDir\agent" | Out-Null
